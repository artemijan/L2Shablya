...
impl NewCrypt {
    ...
    fn calculate_checksum_block(raw: &[u8], offset: usize, size: usize) -> (u32, u32) {
        ...
        while i < count {
            ...
            check |= (u32::from(raw[i + 2]) << 0x10) & 0xff0000;
            check |= (u32::from(raw[i + 3]) << 0x18) & 0xff000000;
            chksum ^= check;
            i += 4;
        }
        check = u32::from(raw[i]) & 0xff;
    ...
    pub fn decrypt(&self, raw: &mut [u8], offset: usize, size: usize) -> Result<(), PacketErrors> {
        if size % 8 > 0 || offset + size > raw.len() {
            return Err(PacketErrors::DecryptBlowfishError);
        }
...
mod test {
    ...
    fn test_decrypt_auth_gg() {
        ...
        let decryptor = NewCrypt::from_u8_key(&key);
    ...
    fn test_append_checksum() {
        ...
        let size = data.len() - 2;
        NewCrypt::append_checksum(&mut data, 2, size);
    ...
    fn test_verify_checksum() {
        ...
        let result = NewCrypt::verify_checksum(&data, 2, data.len() - 2);
    ...
    fn verify_checksum_2() {
        let data = [
            ...
            246, 143, 230, 10, 163, 117, 66, 16, 5, 0, 0, 0, 49, 0, 48, 0, 46, 0, 51, 0, 55, 0,
            46, 0, 49, 0, 50, 0, 57, 0, 46, 0, 48, 0, 47, 0, 50, 0, 52, 0, 0, 0, 49, 0, 48, 0, 46,
            0, 51, 0, 55, 0, 46, 0, 49, 0, 50, 0, 57, 0, 46, 0, 50, 0, 0, 0, 49, 0, 48, 0, 46, 0,
            50, 0, 49, 0, 49, 0, 46, 0, 53, 0, 53, 0, 46, 0, 48, 0, 47, 0, 50, 0, 52, 0, 0, 0, 49,
            0, 48, 0, 46, 0, 50, 0, 49, 0, 49, 0, 46, 0, 53, 0, 53, 0, 46, 0, 50, 0, 0, 0, 49, 0,
            57, 0, 50, 0, 46, 0, 49, 0, 54, 0, 56, 0, 46, 0, 50, 0, 48, 0, 46, 0, 48, 0, 47, 0, 50,
            0, 52, 0, 0, 0, 49, 0, 57, 0, 50, 0, 46, 0, 49, 0, 54, 0, 56, 0, 46, 0, 50, 0, 48, 0,
            46, 0, 49, 0, 48, 0, 50, 0, 0, 0, 49, 0, 50, 0, 55, 0, 46, 0, 48, 0, 46, 0, 48, 0, 46,
            0, 48, 0, 47, 0, 56, 0, 0, 0, 49, 0, 50, 0, 55, 0, 46, 0, 48, 0, 46, 0, 48, 0, 46, 0,
            49, 0, 0, 0, 48, 0, 46, 0, 48, 0, 46, 0, 48, 0, 46, 0, 48, 0, 47, 0, 48, 0, 0, 0, 56,
            0, 57, 0, 46, 0, 50, 0, 51, 0, 52, 0, 46, 0, 50, 0, 52, 0, 53, 0, 46, 0, 49, 0, 54,
            0, 0, 0, 0, 0, 0, 0, 133, 132, 217, 23
        ];
        let result = NewCrypt::verify_checksum(&data, 0, data.len());
        assert!(result, "Verify checksum must work");
    }
