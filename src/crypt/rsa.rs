use crate::common::errors::RsaErrors;
...
impl ScrambledRSAKeyPair {
    ...
    pub fn decrypt_data(&self, encrypted_data: &[u8]) -> Result<Vec<u8>, ErrorStack> {
        ...
        let decrypted_len =
            pkey.private_decrypt(encrypted_data, &mut decrypted_data, Padding::NONE)?;
        decrypted_data.truncate(decrypted_len);
        Ok(decrypted_data)
    }

    #[allow(unused)]
    pub fn from_pem(private_key_pem: &str) -> Result<PKey<openssl::pkey::Private>, RsaErrors> {
        let private_key = Rsa::private_key_from_pem(private_key_pem.as_bytes())
            .map_err(|_| RsaErrors::ErrorReadingPem)
            .unwrap();
        PKey::from_rsa(private_key).map_err(|_| RsaErrors::ErrorReadingPem)
    }
}
