...
impl UserInfo {
    ...
    fn write_current_hp_mp_cp_exp_sp(
        ...
        let next_threshold = exp_table.get_exp(u8::try_from(char_info.char_model.level)?);
        let gained = char_info.char_model.exp - current_threshold;
        let total = next_threshold - current_threshold;
        #[allow(clippy::cast_precision_loss)]
        self.buffer.write_f64(gained as f64 / total as f64)?;
        Ok(())
    }
    ...
    fn write_stats(&mut self, player: &Player) -> anyhow::Result<()> {
        ...
        self.buffer
            .write_u16(player.get_weapon().map_or(20u16, |_| 40u16))?;
